...
from soundspaces.utils import load_metadata, convert_semantic_object_to_rgb
from soundspaces.mp3d_utils import HouseReader
from ss_baselines.common.utils import get_spectrogram

def convert_label(semantic, mapping, rgb=None):
    mapping[mapping < 0] = 41
    semantic = mapping[semantic]

    semantic[semantic == 16] = 2
    semantic[(semantic != 1) & (semantic != 2) & (semantic != 17)] = 3
    semantic[semantic == 17] = 0
    semantic = torch.from_numpy(semantic)
    semantic = F.one_hot(semantic, 4)
...
class SoundSpacesSim(Simulator, ABC):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._env_time = 0
        # The sounds used in the training splits
        self._other_sounds = ['apollo', 'exhaust_fan', 'person_6', 'engine_3', 'person_3', 'air_conditioner', 'elevator_door_close', 'waves3', 'turbine_2', 'person_1', 'computer_beeps', 'metaldooropen', 'copy_machine', 'fountain', 'water_waves_1', 'plane_old', 'canon_short', 'waves1', 'steam_door', 'police_siren', 'elevator', 'helicopter2', 'whirr_loop', 'lawrence', 'bell', 'evolves', 'air_pump', 'birds4', 'person_0', 'electric_buzz', 'fan_2', 'horn_1', 'person_2', 'metaldoorclose', 'birds3', 'person_4', 'engine_1', 'force_field_loop', 'alien_ambience', 'turbine_3', 'machinery_loop', 'waves2', 'chiller', 'big_door', 'angel', 'static', 'computer_fan_2', 'birds2', 'car_start_run', 'come_to_office', 'bowl', 'turbine_1', 'siren', 'fan_1', 'air_compressor_clicks', 'sweet_beat', 'impulse', 'ship_ambience', 'fireplace', 'computer_fan_1', 'engine_2', 'wooddoorclose', 'big_bass_rattle', 'fan_3', 'person_5', 'birds1', 'horn_beeps', 'alarm', 'plane', 'test', 'big_engine_loop', 'machinery', 'beep_noise_loop']
        self._other_sounds = [x + '.wav' for x in self._other_sounds]
        self._ori_source = None

        self._other_goals = dict()
        self._alternative_sound = None
        self._alternative_goal = None
        self._mix = False
        self._alpha = 1
        if 'EVAL_RUN' not in config:
            self.new_exp = False
        else:
            self.new_exp = True
            self._rir = None
            self._eval = config.EVAL_RUN
            self._mix = config.MIX
            if self._mix:
                print("Mixed Sound Augmentation Used")
    ...
    def current_scene_name(self):
        if 'replica' in self._current_scene:
    ...
    @property
    def current_alternative_sound(self):
        return self._source_sound_dict[self._alternative_sound]
    ...
    def reconfigure(self, config: Config) -> None:
        ...
        if not is_same_sound: # Sounds are only load in this function
            ...
            for i in range(len(self._other_sounds)):
                self._alternative_sound = self._other_sounds[i]
                self._load_alternative_sound()
    ...
    def switch_sound(self):
        ...
        if self._eval:
            return
        if self._mix:
            choices = set(range(0, len(self._other_sounds)))
            index_1, index_2 = random.sample(list(choices), 2)
            sound_1 = self._source_sound_dict[self._other_sounds[index_1]]
            sound_2 = self._source_sound_dict[self._other_sounds[index_2]]
            # Invert 
            invert_1 = bool(random.getrandbits(1))
            invert_2 = bool(random.getrandbits(1))
            if invert_1:
                sound_1 = sound_1[::-1]
            if invert_2:
                sound_2 = sound_2[::-1]

            mix_rate = random.betavariate(self._alpha, self._alpha)
            new_source = mix_rate * sound_1 + (1-mix_rate) * sound_2

            self._ori_source = self._source_sound_dict[self._current_sound]
            self._current_sound = 'mix'
            self._source_sound_dict['mix'] = new_source
            return 
    ...
    def reset(self):
        ...
        if 'SEMANTIC_SENSOR' in self.config.AGENT_0.SENSORS and not self.config.USE_RENDERED_OBSERVATIONS:

            self.compute_semantic_index_mapping()

            observations['semantic'] = convert_label(observations['semantic'], self._instance2label_mapping)
        if self.new_exp and not self._eval:
            self.switch_sound()
    ...
    def step(self, action, only_allowed=True):
        ...
        # if 'SEMANTIC_SENSOR' in self.config.AGENT_0.SENSORS :
        if 'SEMANTIC_SENSOR' in self.config.AGENT_0.SENSORS and not self.config.USE_RENDERED_OBSERVATIONS:
            self.compute_semantic_index_mapping()
            print("semantic sensor activated, exit and doublecheck")
            exit()
        return observations
    ...
    def _load_source_sounds(self):
        # load all mono files at once. Not seemed to be used in baseline
    ...
    def _load_alternative_sound(self):
        if self._alternative_sound not in self._source_sound_dict:
            audio_data, sr = librosa.load(os.path.join(self.source_sound_dir, self._alternative_sound),
                                          sr=self.config.AUDIO.RIR_SAMPLING_RATE)
            self._source_sound_dict[self._alternative_sound] = audio_data
    ...
    def _compute_alternative_audiogoal(self):
        sampling_rate = self.config.AUDIO.RIR_SAMPLING_RATE
        binaural_rir = self._rir
        if self.current_source_sound.shape[0] == sampling_rate:
            """ # Comment this part to only return the target sound instead of all
            for i in range(len(self._other_sounds)):
                self._alternative_sound = self._other_sounds[i]
                a_binaural_convolved = np.array([fftconvolve(self.current_alternative_sound, binaural_rir[:, channel]
                                                        ) for channel in range(binaural_rir.shape[-1])])
                self._alternative_goal = a_binaural_convolved[:, :sampling_rate]
                self._other_goals[self._alternative_sound] = self._alternative_goal
            """
            choices = set(range(0, len(self._other_sounds))) - set([self._source_index])
            if self._mix:
                index_1, index_2 = random.sample(list(choices), 2)
                sound_1 = self._source_sound_dict[self._other_sounds[index_1]]
                sound_2 = self._source_sound_dict[self._other_sounds[index_2]]
                ...
                if invert_2:
                    sound_2 = sound_2[::-1]
                mix_rate = random.betavariate(self._alpha, self._alpha)
                new_target = mix_rate * sound_1 + (1-mix_rate) * sound_2
                a_binaural_convolved = np.array([fftconvolve(new_target, binaural_rir[:, channel]
                                                    ) for channel in range(binaural_rir.shape[-1])])
            else:
                target_index = random.choice(list(choices))
                self._alternative_sound = self._other_sounds[target_index]
                a_binaural_convolved = np.array([fftconvolve(self.current_alternative_sound, binaural_rir[:, channel]
                                                    ) for channel in range(binaural_rir.shape[-1])])
            self._alternative_goal = a_binaural_convolved[:, :sampling_rate]
        else:
            ...
            exit()
        if self._eval and self.new_exp:
            if self._current_sound in self._other_sounds:
                print("Heard Sounds Appears During Evaluation")
                exit()
    ...
    def _compute_audiogoal(self):
        ...
        if self._episode_step_count > self._duration:
            ...
            try:
                ...
                success_flag = False
            # Handler for file read failure on my device specifically, can be removed 
            if not success_flag:
                old_dir = '/mnt/sda/sound-space/' + str(binaural_rir_file)
                if 'replica' in self._current_scene:
                    old_dir = binaural_rir_file.replace('sdb', 'sda')
                try:
                    sampling_freq, binaural_rir = wavfile.read(old_dir)  # float32
                except (ValueError, OSError) as e:
                    logging.warning("{} file is still not readable".format(old_dir))
    ...
    def get_current_spectrogram_observation(self, audiogoal2spectrogram):
        if self.config.AUDIO.HAS_DISTRACTOR_SOUND:
            ...
            joint_index = (self._source_position_index, self._receiver_position_index, self.azimuth_angle)
            # disable the spectrogram cache which occassionally produce errors
    ...
    def get_sound_source_spectrograms(self, audiogoal2spectrogram):
        audios = np.stack([self.current_source_sound, self.current_alternative_sound], axis=0)
        spectrograms = audiogoal2spectrogram(audios)
        return spectrograms

    def get_echoes_spectrograms(self, tospectrogram):
        binaural_rir_file = os.path.join(self.binaural_rir_dir, str(self.azimuth_angle), '{}_{}.wav'.format(
            self._source_position_index, self._source_position_index))
        try:
            sampling_freq, binaural_rir = wavfile.read(binaural_rir_file)  # float32
        except (ValueError, OSError) as e:
            logging.warning("{} file is not readable".format(binaural_rir_file))
            binaural_rir = np.zeros((16000, 2)).astype(np.float32)
        if len(binaural_rir) == 0:
            logging.debug("Empty RIR file at {}".format(binaural_rir_file))
            binaural_rir = np.zeros((16000, 2)).astype(np.float32)

        binaural_convolved = np.array([fftconvolve(self.sweep, binaural_rir[:, channel]
                                                           ) for channel in range(binaural_rir.shape[-1])])
        spectrograms = tospectrogram(binaural_convolved)
        return spectrograms
    ...
    def get_rir_spectrograms(self, audiogoal2spectrogram):
        sr = 16000
        if self._rir.shape[0] <= sr:
            pad = np.zeros((sr-self._rir.shape[0], 2))
            self._rir = np.concatenate([self._rir, pad])
        else:
            self._rir = self._rir[:sr, :]
        self._rir = self._rir.transpose(1, 0)
        spectrograms = audiogoal2spectrogram(self._rir)

        return spectrograms

    def get_alternatives_audio_observation(self):
        other_goals = self._compute_alternative_audiogoal()
        return other_goals # A dictionary before update code. Now it's a audio 

    def get_alternatives_spectrogram_observation(self, audiogoal2spectrogram):
        other_goals = self.get_alternatives_audio_observation()
        spectrogram = audiogoal2spectrogram(other_goals)
        return spectrogram
