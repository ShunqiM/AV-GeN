...
@registry.register_sensor
class SwitchAudioSensor(Sensor):
    def __init__(self, *args: Any, sim: Simulator, config: Config, **kwargs: Any):
        self._sim = sim
        super().__init__(config=config)

    def _get_uuid(self, *args: Any, **kwargs: Any):
        return "switch_audiogoal"

    def _get_sensor_type(self, *args: Any, **kwargs: Any):
        return SensorTypes.PATH

    def _get_observation_space(self, *args: Any, **kwargs: Any):
        # sensor_shape = (self._sim.config.AUDIO.NUM_ALTERNATIVES ,2, self._sim.config.AUDIO.RIR_SAMPLING_RATE)
        sensor_shape = (2, self._sim.config.AUDIO.RIR_SAMPLING_RATE)
    ...
    def get_observation(self, *args: Any, observations, episode: Episode, **kwargs: Any):
        audio_dic = self._sim.get_alternatives_audio_observation()
        return audio_dic

@registry.register_sensor
class SoundSourceSensor(Sensor):
    cls_uuid: str = "sound_source"
    def __init__(self, *args: Any, sim: Simulator, config: Config, **kwargs: Any):
        self._sim = sim
        super().__init__(config=config)
    ...
    def get_observation(self, *args: Any, observations, episode: Episode, **kwargs: Any):
        spectrogram = self._sim.get_sound_source_spectrograms(self.compute_spectrogram)
...
@registry.register_sensor
class SwitchSpectrogramSensor(Sensor):
    cls_uuid: str = "switch_spectrogram"
    def __init__(self, *args: Any, sim: Simulator, config: Config, **kwargs: Any):
        self._sim = sim
        super().__init__(config=config)

    def _get_uuid(self, *args: Any, **kwargs: Any):
        return "switch_spectrogram"
    ...
    def _get_observation_space(self, *args: Any, **kwargs: Any):
        ...
        # spectrogram = np.repeat(spectrogram, self._sim.config.AUDIO.NUM_ALTERNATIVES, axis=0)
        return spaces.Box(
            low=np.finfo(np.float32).min,
            high=np.finfo(np.float32).max,
            shape=spectrogram.shape,
    ...
    def get_observation(self, *args: Any, observations, episode: Episode, **kwargs: Any):
        spectrogram = self._sim.get_alternatives_spectrogram_observation(self.compute_spectrogram)
        return spectrogram
