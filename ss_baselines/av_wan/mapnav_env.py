...
class MapNavEnv(habitat.RLEnv):
    ...
    def step(self, *args, **kwargs):
        ...
        # The dump variable is used for logging intermediate actions
        if "dump" in kwargs:
            dump = kwargs["dump"]
        else:
            dump = False
        ...
        if dump:
            with open('tmp_actions.txt', 'w') as f:
                for i in traj:
                    f.write('%d \n' % i)
                # json.dump(traj, my_list_file)
        #     return observation, cumulative_reward, done, info, traj
