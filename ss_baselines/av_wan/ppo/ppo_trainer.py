...
class PPOTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, observation_space=None) -> None:
        ...
        self.agent = PPO(
            ...
            ssound = ppo_cfg.ss_sound,
            ss_coef = ppo_cfg.ss_coef
        )
    ...
    def _update_agent(self, ppo_cfg, rollouts):
        ...
        value_loss, action_loss, dist_entropy, ss_loss = self.agent.update(rollouts)
    ...
    def train(self) -> None:
        ...
        with TensorboardWriter(
            ...
            for update in range(start_update, self.config.NUM_UPDATES):
                ...
                delta_pth_time, value_loss, action_loss, dist_entropy, ss_loss = self._update_agent(
                    ppo_cfg, rollouts
                )
                pth_time += delta_pth_time
                ...
                # )
                writer.add_scalar(
                    'Policy/SS_Loss', ss_loss, count_steps
                )
                writer.add_scalar(
    ...
    def _eval_checkpoint(
        ...
        while (
            ...
            if dump:
                outputs = self.envs.step([{"action": actions[0][0].item(), "dump": True}])
                observations, rewards, dones, infos = [list(x) for x in zip(*outputs)]
                with open("tmp_actions.txt", "r", encoding="utf-8") as g:
                    traj_step = list(map(int, g.readlines()))    
                if scene_episode_id not in trajs:
                    trajs[scene_episode_id] = traj_step
                else:
                    trajs[scene_episode_id].extend(traj_step)
            ...
            for i in range(n_envs):
                ...
                if not_done_masks[i].item() == 0:
                    ...
                    for metric_uuid in self.metric_uuids:
                        ...
                        # agg_metrics[metric_uuid] += infos[i][metric_uuid]
        ...
        if dump:
            ...
            for eval_dct_key in EVAL_DCT_KEY_TO_METRIC_NAME:
                assert EVAL_DCT_KEY_TO_METRIC_NAME[eval_dct_key] in episode_metrics_mean
                eval_dct[eval_dct_key] = float(f"{episode_metrics_mean[EVAL_DCT_KEY_TO_METRIC_NAME[eval_dct_key]]:.2f}")
            with open(os.path.join(config.TENSORBOARD_DIR,'wan_traj.json'), "w") as fo:
        ...
        if True: # Always try to log the result
            log_split = config.EVAL.SPLIT
            # log_split = log_split.replace('test', 'val')
            writer.add_scalar("{}/reward".format(log_split), episode_reward_mean, checkpoint_index)
            writer.add_scalar("{}/reaching_waypoint".format(log_split), episode_reaching_waypoint_mean,
                              checkpoint_index)
            writer.add_scalar("{}/cant_reach_waypoint".format(log_split), episode_cant_reach_waypoint_mean,
                              checkpoint_index)
            for metric_uuid in self.metric_uuids:
                writer.add_scalar(f"{log_split}/{metric_uuid}", episode_metrics_mean[metric_uuid],
                                  checkpoint_index)
