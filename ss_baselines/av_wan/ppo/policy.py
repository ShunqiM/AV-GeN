...
class AudioNavSSNet(Net):
    ...
    def __init__(self, observation_space, hidden_size, goal_sensor_uuid, encode_rgb, encode_depth):
        ...
        self.ss_sound = True
        self.batch_size = 256
        # The projection head could be added for potentially better performance
        # self.projection = nn.Sequential(nn.Linear(512, 256), nn.ReLU(inplace=True), nn.Linear(256, 256))
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks, synthesis=False):
        ...
        if synthesis:
            ...
            # If a projection head is used for contrastive loss computation
            if hasattr(self, 'projection'):
                target_batch = self.projection(target_batch)
                predicted = self.projection(predicted)
